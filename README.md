...
### Texture / UV obfuscation

- **Remap UV Textures** — for every material on the avatar, picks a
  deterministic flip mode (FlipX / FlipY / FlipBoth), regenerates each
  Texture2D bound to that material with the flip baked in, and rewrites
  the affected mesh's UV0 so the visual result is unchanged. The output
  textures are byte-different from the originals, so a ripper extracting
  your avatar can no longer match them against asset-store textures by
  content hash.

  Skipped (with a console warning) for renderers where two submeshes share
  vertices across different materials — a single vertex cannot carry two
  conflicting flips.
...
- **NOT a mesh encryption tool.** The geometry (vertices / triangles /
  bones) is left intact. We only rewrite UVs and texture bytes. Pair this
  with a dedicated mesh-encryption tool (HE-Vrcat / IRIS / Bake Defender /
...
### Console warns "UV remap skipped for renderer ..."

The mesh isn't Read/Write enabled, or two of its submeshes share vertices
across different materials. The first case: tick Read/Write on the mesh
importer. The second: rare, will skip those materials' UV remap (other
materials still get remapped, the avatar still works fine).
...
- `Remap UV Textures` requires meshes to be Read/Write enabled — meshes
  that aren't will simply skip the texture remap (the avatar still uploads
  fine, just without the texture obfuscation for those meshes).
...
└── Editor/                        ← NDMF pipeline
    ...
    │   ├── PathRemapper.cs
    │   ├── AssetCloner.cs
    │   ├── UVTextureRemapper.cs    (per-material UV flip)
