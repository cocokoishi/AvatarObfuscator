...
此外，这个版本针对assetripper等软件解包出来的屎的修复，具体而言是部分由assetripper导出的animator参数曲线使用了哈希等价占位名，而我们程式的原逻辑仅按参数字符串精确匹配，导致控制器参数混淆重命名后，相关动画曲线仍绑定旧参数哈希，无法驱动新的参数。这是一个用户偶然报告的问题，即使不修复也不会影响99.61%用户的体验，但是我还是选择修复了，因为我额度太多了，我让llm自己独立自主的找到了这个问题的根本原因，没有人为干涉。我无意提升盗模者的使用体验，但是我把这次非常深层次bug的修复视为了对索嗨模型的优质的测试。
