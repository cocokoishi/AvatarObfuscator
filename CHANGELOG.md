...
可以确定，这种哈希化后的层级路径只会出现在AssetRipper吐出来的Dummy结果里，永远不会存在于正常工程文件中。
所以我不会再为这种场景做任何维护了，别指望我会给盗模狗提升任何体验，之前的修复只是在测试某新llm的性能，事实证明性能还行，不过也只是无情的任务执行机器，在阅读代码之前他的世界知识里面完全没有上述的知识的概念。
如果还是不行，那就自己关掉 hierarchy gameobject obf 这个选项，以及形态键等一切可能影响这玩意的东西。
很遗憾我无法解决这个问题。但是连AssetRipper也无法解决这个问题：他也只是遍历猜，猜不到路径就CRC32Hash碰撞做到等效路径，那我们的ai slop也不可能解决了（嗯其实我只是不想把钱浪费在这种没有好处的地方），不过可以搭配某脚本的某脚本使用，他会把assetripper没猜到的哈希原版再猜一次，可以再解决90%以上没猜到的内容。
除此之外，这个混淆器已经进入尾声，在这3个月的开发过程中，本脚本的100%代码均由AI编写。

## [0.4.7] - 2026-07-15
我们添加了更多的混淆，虽然我觉得一些名字完全没必要混淆但是我还是决定在这个版本应用这些混淆。
同时我们大幅优化了性能，在我的工程文件下，原版模型需要2分钟打包，而挂载之前版本的混淆器之后需要大约6分钟。现在优化之后的混淆已经不会造成超过10%的额外时间消耗。
| 项目 | 优化前 | 最新 | 节省 |
|---|---:|---:|---:|
| 共享资产克隆 | 65–73 秒 | 4.996 秒 | 约 60–68 秒 |
| AnimationClip | 349 秒 | 2.579 秒 | 约 346.4 秒 |
| 两项合计 | 414–422 秒 | 7.575 秒 | **约 406–414 秒** |
---
此外，这个版本针对assetripper等软件解包出来的屎的修复，具体而言是部分由assetripper导出的animator参数曲线使用了哈希等价占位名，而我们程式的原逻辑仅按参数字符串精确匹配，导致控制器参数混淆重命名后，相关动画曲线仍绑定旧参数哈希，无法驱动新的参数。这是一个用户偶然报告的问题，即使不修复也不会影响99.61%用户的体验，但是我还是选择修复了，因为我额度太多了，我让llm自己独立自主的找到了这个问题的根本原因，没有人为干涉。我无意提升盗模者的使用体验，因为我觉得有部分prefab也可能会使用哈希的路径从而触发这个bug，虽然这是基本不可能的。但是我把这次非常深层次bug的修复视为了对模型的优质的测试。这是一次愉快的排错：在此我也希望如果你想汇报bug，请你不要对着我自说自话，一会说自己用codex修好了一会又说没修好，给我提供含糊不清自相矛盾的信息，问工程文件也提供不了，这样子我是完全没法下手的，是浪费时间。


## [0.4.6] - 2026-07-12
我们添加了把蓝图id作为混淆后name的前缀的功能，起因是b站上看到居然有能防止hotswap的防盗模插件。在我的看法里，不可能有任何插件能做到让角色在hotswap的时候就直接让软件报错，那个视频尝试把蓝图id拉的到处都是的同时添加金丝雀shader（似乎是在假设hotswap后模型的贴图会被重新压缩，shader写死逻辑检测特定位置的像素信息来判断是否被篡改），实际上都是无用的，在他的逻辑里面的上传出错实际上是因为他使用了自研的hotswap软件，很明显这个软件写的有问题、因为他逻辑大概率是全局替换蓝图id导致出错的没有在pipelinemanager等地方进行写死的替换，并且我可以肯定他是连unity本地测试的没蓝图id的模型都上传不了的，是典型的在测试集上训练，实际上无法产生任何效果，只能让角色的性能变得更加不行，不过想法是非常有意思的。我们这个混淆插件也有功能更加完备的免费上位替代，不过我们的插件非常懒狗，不仅是唯一开源的ndmf混淆器而且只要一键挂上脚本就能完美享受完全不改变性能的混淆，所以有一定存在价值（虽然是一坨屎山哈哈，不过反倒模还要自己手写插件不就亏了吗）
